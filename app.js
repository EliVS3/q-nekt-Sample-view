 ...
 * Implements actual file export functionality.
...
function exportReport(fmt) {
  ...
  // Get report data
  const reportData = parseReportData(currentRptType);

  // Generate and download file
  switch (fmt.toLowerCase()) {
    case 'pdf':
      exportToPDF(title, reportData);
      break;
    case 'excel':
      exportToExcel(title, reportData);
      break;
    case 'csv':
      exportToCSV(title, reportData);
      break;
  }
...
/**
 * Parses the report HTML content into structured data
 * @param {string} type - Report type
 * @returns {Array} Array of objects with section headers and data rows
 */
function parseReportData(type) {
  const html = RPT_DOC[type] || '';
  const parser = new DOMParser();
  const doc = parser.parseFromString(html, 'text/html');
  const sections = [];
  let currentSection = null;

  // Find all h3 elements (section headers) and their following content
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    } else if (el.classList && el.classList.contains('doc-row') && currentSection) {
 ...
 * @param {Array} data - Report data sections
 */
function exportToPDF(title, data) {
  ...
  data.forEach(section => {
    ...
    section.data.forEach(row => {
      ...
      doc.text(`${row.label}: ${row.value}`, 30, yPosition);
      yPosition += 8;
    });
...
function exportToExcel(title, data) {
  ...
  data.forEach((section, index) => {
    const wsData = [
      [section.title],
      ['Label', 'Value'],
      ...section.data.map(row => [row.label, row.value])
    ];
...
/**
 * Exports the currently visible database table to CSV or Excel
 * @param {string} format - 'csv' or 'xlsx'
 */
function exportTableData(format) {
  // Find the currently visible table
  const visibleTable = document.querySelector('.table-wrap:not([style*="display: none"]) table');
  if (!visibleTable) {
    notify('No table data to export!', 'error', 'Database', 'No visible table found');
    return;
  }
  ...
  rows.forEach(row => {
    ...
    if (rowData.length > 0 && !rowData[0].includes('No records found')) {
      data.push(rowData);
    }
  });

  if (data.length === 0) {
    notify('No data to export!', 'error', 'Database', 'Table contains no records');
    return;
  }

  // Get table name from visible table ID
  const tableId = visibleTable.closest('.table-wrap').id;
  let tableName = 'Database';
  if (tableId === 'db-fac') tableName = 'Faculty Records';
  else if (tableId === 'db-stu') tableName = 'Student Records';
  else if (tableId === 'db-sch') tableName = 'Scholarship Records';
  else if (tableId === 'db-res') tableName = 'Research Records';

  if (format === 'csv') {
    exportTableToCSV(tableName, headers, data);
  } else if (format === 'xlsx') {
    exportTableToExcel(tableName, headers, data);
  }

  notify(`Exported as ${format.toUpperCase()}!`, 'export', 'Database', `${tableName} exported as ${format.toUpperCase()}`);
}
 ...
 */
function exportTableToCSV(tableName, headers, data) {
  ...
  const url = URL.createObjectURL(blob);
  link.setAttribute('href', url);
  link.setAttribute('download', `${tableName.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.csv`);
  link.style.visibility = 'hidden';
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
 ...
 * @param {Array} data - Table data rows
 */
function exportTableToExcel(tableName, headers, data) {
  const wb = XLSX.utils.book_new();
  const wsData = [headers, ...data];
  const ws = XLSX.utils.aoa_to_sheet(wsData);
  XLSX.utils.book_append_sheet(wb, ws, 'Data');
  XLSX.writeFile(wb, `${tableName.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.xlsx`);
}
